...
void CrateApp::AnimateMaterials(const GameTimer& gt)
{
	...
	auto matTrans = flareMat->MatTransform;

//	XMMATRIX rot = XMMatrixRotationZ(gt.TotalTime());

//	XMStoreFloat4x4(&flareMat->MatTransform, rot);

	//float& tu = flareMat->MatTransform(3, 0);
	//float& tv = flareMat->MatTransform(3, 1);
	...
	//flareMat->MatTransform(3, 0) = tu;
	//flareMat->MatTransform(3, 1) = tv;
...
void CrateApp::LoadTextures()
{
	...
	flareTex->Filename = L"../../Textures/Exercise_Media/flare.dds";
	...
	flarealphaTex->Filename = L"../../Textures/Exercise_Media/flarealpha.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), flarealphaTex->Filename.c_str(),
		flarealphaTex->Resource, flarealphaTex->UploadHeap));

	mTexture[flareTex->Name] = std::move(flareTex);
	mTexture[flarealphaTex->Name] = std::move(flarealphaTex);
}
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	auto flareTex = mTexture["flareTex"]->Resource;
	...
	auto flarealphaTex = mTexture["flarealphaTex"]->Resource;
