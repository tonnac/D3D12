...
void BlendApp::Draw(const GameTimer& gt)
{
	...
	if (!misShowNormal)
	{
		...
		mCommandList->SetPipelineState(mPSOs["alphaTestedP"].Get());
	...
	if (!misShowNormal)
	{
		...
		mCommandList->SetPipelineState(mPSOs["transparentP"].Get());
...
void BlendApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVSP"] = d3dUtil::CompileShader(L"Normal.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePSP"] = d3dUtil::CompileShader(L"Normal.hlsl", nullptr, "PS", "ps_5_0");
	mShaders["alphaTestedPSP"] = d3dUtil::CompileShader(L"Normal.hlsl", nullptr, "PS", "ps_5_0");
...
void BlendApp::BuildPSOs()
{
	...
	opaquePsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["standardVSP"]->GetBufferPointer()),
		mShaders["standardVSP"]->GetBufferSize()
	};
	opaquePsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["opaquePSP"]->GetBufferPointer()),
		mShaders["opaquePSP"]->GetBufferSize()
	};
	opaquePsoDesc.GS =
	{
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&opaquePsoDesc, IID_PPV_ARGS(mPSOs["opaqueP"].GetAddressOf())));
	...
	transparentPsoDesc.BlendState.RenderTarget[0] = transparencyBlendDesc;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&transparentPsoDesc, IID_PPV_ARGS(mPSOs["transparentP"].GetAddressOf())));

	alphaTestedPsoDesc = opaquePsoDesc;
	alphaTestedPsoDesc.PS =
	{
		 reinterpret_cast<BYTE*>(mShaders["alphaTestedPSP"]->GetBufferPointer()),
		 mShaders["alphaTestedPSP"]->GetBufferSize()
	};
	alphaTestedPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&alphaTestedPsoDesc, IID_PPV_ARGS(mPSOs["alphaTestedP"].GetAddressOf())));
}
