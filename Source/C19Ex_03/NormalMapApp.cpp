...
void NormalMapApp::Update(const GameTimer& gt)
{
	...
	static float beforedll = 0.0f;
	dll += gt.DeltaTime() * 0.125f * XM_PI;
	...
	for (auto&m : mRitemLayer[(int)RenderLayer::Opaque])
	{
		XMMATRIX scale = XMMatrixScaling(1.5f + cosf(gt.TotalTime()) * 0.5f + 0.5f, 1.2f, 0.9f);
		XMFLOAT4X4 world = m->World;
		XMFLOAT3 Pos;
		...
		XMMATRIX rot = XMMatrixRotationY(dll - beforedll);
		XMStoreFloat4x4(&m->World, scale * t * rot);
