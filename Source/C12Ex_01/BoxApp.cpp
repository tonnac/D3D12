...
private:
	...
	std::unique_ptr<MeshGeometry> mLineGeo = nullptr;
...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->IASetVertexBuffers(0, 1, &mLineGeo->VertexBufferView());
	mCommandList->IASetIndexBuffer(&mLineGeo->IndexBufferView());
	mCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_LINESTRIP);

	mCommandList->SetGraphicsRootDescriptorTable(0, mCbvHeap->GetGPUDescriptorHandleForHeapStart());

	mCommandList->DrawIndexedInstanced(mLineGeo->DrawArgs["line"].IndexCount, 1, 0, 0, 0);
...
void BoxApp::BuildLineGeometry()
{
	constexpr int sliceCount = 3;
	constexpr float thetaStep = XM_2PI / sliceCount;

	std::array<Vertex, sliceCount> vertices;
	std::array<std::uint16_t, sliceCount + 1> indicies;

	for (int i = 0; i < sliceCount; ++i)
	{
		float theta = thetaStep * i;
		vertices[i].Pos = { cosf(theta), 0.0f, sinf(theta) };
		if(i % 2 == 0)
