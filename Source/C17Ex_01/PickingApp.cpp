...
private:
	...
	bool Intersects(const DirectX::BoundingBox & box, DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Dir, float & Dist);
...
void PickingApp::LoadTextures()
{
	...
	mTexture[defaultDiffuseTex->Name] = std::move(defaultDiffuseTex);
}
...
void PickingApp::BuildDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors = (UINT)mTexture.size();
	...
	auto defaultDiffuseTex = mTexture["defaultDiffuseTex"]->Resource;
...
void PickingApp::BuildCarGeometry()
{
	...
	BoundingSphere spheres;
	XMStoreFloat3(&spheres.Center, C);
	spheres.Radius = XMVectorGetX(XMVector3Length(C - vMax));
...
void PickingApp::BuildPSOs()
{
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&highlightPsoDesc, IID_PPV_ARGS(mPSOs["highlight"].GetAddressOf())));
}
...
void PickingApp::BuildMaterials()
{
	...
	mMaterials[gray0->Name] = std::move(gray0);
	mMaterials[highlight0->Name] = std::move(highlight0);
}
...
void PickingApp::BuildRenderItems()
{
	...
	carRitem->Spheres = carRitem->Geo->DrawArgs["car"].Sphere;
...
void PickingApp::Pick(int sx, int sy)
{
	...
	for (auto ri : mRitemLayer[(int)RenderLayer::Opaque])
	{
		...
		float tmin = 0.0f;
		auto spheres = geo->DrawArgs["car"].Sphere;
		//if (ri->Bounds.Intersects(rayOrigin, rayDir, tmin) == true)
		if(ri->Spheres.Intersects(rayOrigin, rayDir, tmin) == true)
		{
...
bool PickingApp::Intersects(const DirectX::BoundingBox & box, DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Dir, float & Dist)
{
	XMVECTOR c = XMLoadFloat3(&box.Center);
	XMVECTOR e = XMLoadFloat3(&box.Extents);

	XMVECTOR max = c + e;
	XMVECTOR min = c - e;

	XMVECTOR InverseAxisDirection = XMVectorReciprocal(Dir);
	XMVECTOR t1 = XMVectorMultiply(XMVectorSubtract(min, Origin), InverseAxisDirection);
	XMVECTOR t2 = XMVectorMultiply(XMVectorSubtract(max, Origin), InverseAxisDirection);

	XMVECTOR t_min = XMVectorMin(t1, t2);
	XMVECTOR t_max = XMVectorMax(t1, t2);

	t_min = XMVectorMax(XMVectorMax(t_min, XMVectorSplatY(t_min)), XMVectorSplatZ(t_min));
	t_max = XMVectorMin(XMVectorMin(t_max, XMVectorSplatY(t_max)), XMVectorSplatZ(t_max));

	float t_maxf = XMVectorGetX(t_max);
	float t_minf = XMVectorGetX(t_min);
