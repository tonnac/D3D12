...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	XMStoreFloat4x4(&mReflectedFloorRitem->World, floorWorld * R);
...
void StencilApp::UpdateScreenPassCB(const GameTimer& gt)
{
	...
	XMStoreFloat4x4(&mScreenPassCB.View, XMMatrixTranspose(view));
	XMStoreFloat4x4(&mScreenPassCB.InvView, XMMatrixTranspose(invView));
	XMStoreFloat4x4(&mScreenPassCB.Proj, XMMatrixTranspose(proj));
	XMStoreFloat4x4(&mScreenPassCB.InvProj, XMMatrixTranspose(invProj));
	XMStoreFloat4x4(&mScreenPassCB.ViewProj, XMMatrixTranspose(viewProj));
	XMStoreFloat4x4(&mScreenPassCB.InvViewProj, XMMatrixTranspose(invViewProj));
	mScreenPassCB.EyePosW = mEyePos;
	mScreenPassCB.RenderTargetSize = XMFLOAT2((float)mClientWidth, (float)mClientHeight);
	mScreenPassCB.InvRenderTargetSize = XMFLOAT2(1.0f / mClientWidth, 1.0f / mClientHeight);
	mScreenPassCB.NearZ = 1.0f;
	mScreenPassCB.FarZ = 1000.0f;
	mScreenPassCB.TotalTime = gt.TotalTime();
	mScreenPassCB.DeltaTime = gt.DeltaTime();

	auto currPassCB = mCurrFrameResource->PassCB.get();
	currPassCB->CopyData(2, mScreenPassCB);
}
...
void StencilApp::LoadTextures()
{
	...
	mTexture[checkboardTex->Name] = std::move(checkboardTex);
	mTexture[iceTex->Name] = std::move(iceTex);
	mTexture[white1x1Tex->Name] = std::move(white1x1Tex);
}
...
void StencilApp::BuildPSOs()
{
	...
	reflectionDSS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	reflectionDSS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	reflectionDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;

	reflectionDSS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	reflectionDSS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	reflectionDSS.BackFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	reflectionDSS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
	...
	shadowDSS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	...
	shadowDSS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	...
	screenPsoDesc.DepthStencilState.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
...
void StencilApp::BuildFrameResources()
{
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(),
			3, (UINT)mAllRitems.size(), (UINT)mMaterials.size()));
	}
}
