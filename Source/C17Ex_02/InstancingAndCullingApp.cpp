...
void InstancingAndCullingApp::LoadTextures()
{
	...
	mTexture[grassTex->Name] = std::move(grassTex);
	mTexture[defaultTex->Name] = std::move(defaultTex);
}
...
void InstancingAndCullingApp::BuildDescriptorHeaps()
{
	...
	auto iceTex = mTexture["iceTex"]->Resource;
	auto grassTex = mTexture["grassTex"]->Resource;
	auto defaultTex = mTexture["defaultTex"]->Resource;
...
void InstancingAndCullingApp::BuildSkullGeometry()
{
	...
	const UINT ibByteSize = sizeof(std::uint32_t) * (UINT)indices.size();
...
void InstancingAndCullingApp::BuildMaterials()
{
	...
	mMaterials[ice0->Name] = std::move(ice0);
	mMaterials[grass0->Name] = std::move(grass0);
...
void InstancingAndCullingApp::Pick(int sx, int sy)
{
	...
	auto& instanceData = mAllRitems[0]->Instances;
	...
	for (UINT i = 0; i < (UINT)instanceData.size(); ++i)
	{
		...
		float tmin = 0.0f;
		if (Intersects(mAllRitems[0]->Bounds, rayOrigin, rayDir, tmin))
		{
			instanceData[i].Color = { 1.0f, 0.0f, 0.0f, 1.0f };
		}
		//if (mAllRitems[0]->Bounds.Intersects(rayOrigin, rayDir, tmin))
		//{
		//	instanceData[i].Color = { 1.0f, 0.0f, 0.0f, 1.0f };
		//}
	}
...
bool InstancingAndCullingApp::Intersects(const DirectX::BoundingBox & box, DirectX::FXMVECTOR Origin, DirectX::FXMVECTOR Dir, float & Dist)
{
