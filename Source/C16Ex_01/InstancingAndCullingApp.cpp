...
void InstancingAndCullingApp::UpdateInstanceData(const GameTimer& gt)
{
	...
	for (auto&e : mAllRitems)
	{
		...
		for (UINT i = 0; i < (UINT)instanceData.size(); ++i)
		{
			...
			if ((localSpaceFrustum.Contains(e->Spheres) != DirectX::DISJOINT) || (mFrustumCullingEnabled == false))
			{
				InstanceData data;
...
void InstancingAndCullingApp::BuildSkullGeometry()
{
	...
	XMStoreFloat3(&spheres.Center, 0.5f*(vMin + vMax));
	XMVECTOR C = XMLoadFloat3(&spheres.Center);
	float radius = -MathHelper::Infinity;
	for (size_t i = 0; i < vertices.size(); ++i)
	{
		XMVECTOR P = XMLoadFloat3(&vertices[i].Pos);
		radius = MathHelper::Max(radius, XMVectorGetX(XMVector3Length(P - C)));
	}
...
void InstancingAndCullingApp::BuildRenderItems()
{
	...
	skullRitem->Spheres = skullRitem->Geo->DrawArgs["skull"].Sphere;
